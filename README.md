...
| CHD自动抢任务 | 监控 CHDBits 任务名额剩余/上限，定时轮询并按优先级自动报名 |
...
1. 站点管理中添加 CHDBits（ptchdbits.co）并配置 Cookie
2. 插件里选择该站点，设置监控 Cron（建议 `*/1 * * * *`）
3. 勾选「有名额时自动报名」，并设置报名优先级（如 Extreme → Insane → Veteran）
4. 默认开启「达成后停用」：仅监控发现有名额即停；报名成功后若开启进度通知会继续推送进度，否则停用插件
5. 已领取任务时，可按间隔（默认 6 小时）推送上传/下载/做种积分完成进度，通知含用户名
6. 详情页可看：剩余/上限、当前占用、魔力、做种积分、任务档位与报名记录

说明：站点只公开整站总名额（普通会员通常 200），没有每个档位的独立剩余数。进度优先读页面显式数据；否则用报名时账户基线估算增量。
